...
Kitty graphics is requested by default, but Herdr enables it on Windows only after the outer terminal answers its runtime capability query. Until that negotiation succeeds, Herdr emits no Kitty graphics protocol output. Windows Terminal does not advertise the Kitty graphics path Herdr uses, so graphics remain disabled there; compatible terminals such as WezTerm can enable them after a successful query. Set `[terminal].kitty_graphics = false` to disable graphics entirely.

Herdr also negotiates synchronized output before using it to delimit redraws. A terminal that does not answer the mode query keeps the ordinary cursor-hidden redraw path without unadvertised `CSI ? 2026 h/l` sequences.

When the Windows input backend can receive VT replies, Herdr also queries standard OSC default foreground/background colors and the XTWINOPS cell size. Valid replies update the client theme and image geometry; unsupported terminals simply leave the existing defaults unchanged.
