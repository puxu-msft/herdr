...
| Windows ARM64 | `herdr-windows-aarch64.zip` |
...
Stable releases and preview prereleases include architecture-matched Windows archives. Each archive contains `herdr.exe` and its app-local ConPTY runtime. Keep the extracted directory together; do not copy only `herdr.exe`.
